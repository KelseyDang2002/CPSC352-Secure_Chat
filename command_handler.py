...
def handle_commands(client, address, clients, chat_room, key):
        ...
        while True:
                ...
                if command.startswith('chat '):
                    ...
                    if users_offline:
                        ...
                        # Verification
                        send_message(client, 'Start Chatroom Verify'.encode('utf-8'))
                        ...
                        if key_response == str(key):
                            ...
                            break
                        send_message(client, "Incorrect key.".encode('utf-8'))
                elif command == 'join':
                    if alias not in chat_room:
                        # Verification
                        send_message(client, 'Join Verify'.encode('utf-8'))
                        rsa_key = str(rsa_encrypt_data(key, alias))
                        send_message(client, rsa_key.encode('utf-8'))
                        key_response = receive_message(client).decode('utf-8')
                        # Join chat room
